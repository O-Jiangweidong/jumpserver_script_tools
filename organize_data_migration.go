...
func (sa *SimpleAsset) HandleAssetCreate(category string, w *Worker) {
	sa.HandleNodes(w)
...
func (d *Database) HandleAssetCreate(category string, w *Worker) {
	d.HandleNodes(w)
	...
	for _, account := range suFromAccounts {
		...
		_, err = w.jmsClient.CreateAccount(account)
		if err != nil {
			logger.Infof("[迁移资产]资产(%s)下的账号(%s)失败", d.Name, account.Name)
		}
	}
	w.migrateFromAssetMapping[oldAssetID] = newAsset.ID
}
...
func (w *Worker) ParseOption() {
	...
	if opts.MigrateToOrgID == "" {
		logger.Errorf("迁出组织 ID 不能为空")
		os.Exit(1)
	}
	if opts.OtpSecret == "" {
		logger.Errorf("OTP 密钥不能为空，否则账号密码无法同步")
		os.Exit(1)
	}
	w.options = &opts
}
...
func (w *Worker) MigrateUserGroup() {
	...
	w.jmsClient.org = w.migrateToOrg
	var alreadyExistUserGroups = make(map[string]UserGroup)
	for _, userGroup := range w.jmsClient.GetUserGroups() {
		alreadyExistUserGroups[userGroup.Name] = userGroup
	}
	w.jmsClient.org = w.migrateFromOrg
	for _, userGroup := range w.jmsClient.GetUserGroups() {
		if ug, exists := alreadyExistUserGroups[userGroup.Name]; exists {
			w.migrateFromUserGroupMapping[userGroup.ID] = ug.ID
			logger.Infof("[迁移用户组]用户组(%s)已经存在，跳过\n", userGroup.Name)
			continue
		}
		w.jmsClient.org = w.migrateToOrg
...
func (w *Worker) MigrateUser() {
	...
	for _, user := range w.jmsClient.GetUsers() {
		...
		if _, exists := toUserMapping[user.ID]; exists {
			logger.Infof("[迁移用户]组织(%s)下存在用户(%s), 跳过", w.jmsClient.org.Name, user.Name)
			continue
		}
		w.jmsClient.org = w.migrateToOrg
...
func (w *Worker) MigrateAssetSub(category string) {
	...
	w.jmsClient.org = w.migrateToOrg
	var alreadyExistAssets = make(map[string]string)
	for _, a := range w.jmsClient.GetAssets(category) {
		switch category {
		case "hosts":
			asset := a.(Host)
			alreadyExistAssets[asset.Name] = asset.ID
		case "devices":
			asset := a.(Device)
			alreadyExistAssets[asset.Name] = asset.ID
		case "databases":
	...
	for _, a := range w.jmsClient.GetAssets(category) {
		...
		case "hosts":
			asset := a.(Host)
			if assetID, exists := alreadyExistAssets[asset.Name]; exists {
				w.migrateFromAssetMapping[asset.ID] = assetID
				logger.Infof("[迁移资产]资产(%s)已经存在，跳过", asset.Name)
				continue
			}
			asset.HandleAssetCreate(category, w)
			logger.Infof("[迁移资产]资产(%s)到组织(%s)成功\n", asset.Name, w.migrateToOrg.Name)
		case "devices":
			asset := a.(Device)
			if assetID, exists := alreadyExistAssets[asset.Name]; exists {
				w.migrateFromAssetMapping[asset.ID] = assetID
...
func (w *Worker) MigratePerm() {
	...
	w.jmsClient.org = w.migrateToOrg
	var alreadyExistPerms = make(map[string]Perm)
	for _, perm := range w.jmsClient.GetPerms() {
		alreadyExistPerms[perm.Name] = perm
	}
	w.jmsClient.org = w.migrateFromOrg
	for _, perm := range w.jmsClient.GetPerms() {
		w.migrateFromPermMapping[perm.ID] = perm.ID
		if _, exists := alreadyExistPerms[perm.Name]; exists {
			logger.Infof("[迁移授权]授权(%s)已经存在，跳过", perm.Name)
			continue
		}
		var newNodes []Node
